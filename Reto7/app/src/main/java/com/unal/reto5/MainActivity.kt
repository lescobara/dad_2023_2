...
class MainActivity : AppCompatActivity(){
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId){
            ...
            R.id.socket -> {
                val socket = Dialogos.newInstance(Dialogos.DIALOG_TYPE_TREE)
                socket.show(supportFragmentManager,"Conectar")
                true
            }
    ...
    // Function to switch to Socket Fragment
    fun switchToFragmentD() {
        SocketManager.establishConnection()

        val fragmentTransaction = supportFragmentManager.beginTransaction()
        fragmentTransaction.replace(R.id.fragmentContainer, SocketFragment())
        fragmentTransaction.addToBackStack(null) // Add transaction to back stack
        fragmentTransaction.commit()
    }
}