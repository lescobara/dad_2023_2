...
class Dialogos : DialogFragment(){
    ...
    val emptyIntList: ArrayList<Int> = ArrayList()
    ...
    private fun createDialogTwo(): Dialog {
        ...
        dialogView.findViewById<Button>(R.id.option1_button).setOnClickListener {
            //(activity as MainActivity).switchToFragmentA(isLandscape)
            (activity as MainActivity).switchToFragmentA(0,0,emptyIntList,emptyIntList)
            dialog?.dismiss()
        }

        dialogView.findViewById<Button>(R.id.option2_button).setOnClickListener {
            (activity as MainActivity).switchToFragmentB(0,0,emptyIntList,emptyIntList)
            dialog?.dismiss()
        }

        dialogView.findViewById<Button>(R.id.option3_button).setOnClickListener {
            (activity as MainActivity).switchToFragmentC(0,0,emptyIntList,emptyIntList)
            dialog?.dismiss()
        }
