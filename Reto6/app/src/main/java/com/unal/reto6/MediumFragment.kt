...
class MediumFragment : Fragment() {
    ...
    companion object {
        ...
        fun newInstance(player1Count: Int, player2Count: Int,player1Moves: ArrayList<Int>, player2Moves: ArrayList<Int>): MediumFragment {
    ...
    fun makeMove(cell: Int) {
        val buttonSelected: ImageButton? = when (cell) {
            1 -> button1
            2 -> button2
            3 -> button3
            4 -> button4
        ...
        buttonSelected?.let {
            emptyCells.add(cell)
            it.setImageResource(R.drawable.starfilledminor_svgrepo_com)
            player2.add(cell)
            it.isEnabled = false
            soundManager.playRobotSound()
            val checkWinner = checkwinner()
